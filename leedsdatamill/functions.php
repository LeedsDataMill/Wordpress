...
//add_filter('xmlrpc_prepare_post','process_xmlrpc_contnet',10,3);

//function process_xmlrpc_contnet($_post, $post, $fields) {
//  $_post['post_content'] = apply_filters('the_content', $post['post_content']);
//  $_post['post_content'] = str_replace('action="/xmlrpc.php', 'action="/blog', $_post['post_content']);
//  return $_post;
//}
...
function ip_wpcf7_mail_sent($wpcf7)
{
  ...
  {
    header("Location: " . trim($on_sent_ok[0]), true, 302);
    exit;
  }
}
...
// Thumbnail sizes
add_image_size( 'wpbs-featured', 1100, 423, true );
...
if( !function_exists("theme_styles") ) {
	...
	wp_register_style( 'google-fonts', 'http://fonts.googleapis.com/css?family=Roboto+Slab', array(), '1.0', 'all');
	wp_enqueue_style( 'google-fonts' );

	wp_register_style( 'bloom', get_template_directory_uri() . '/library/css/style.css', array(), '1.4', 'all');
	wp_enqueue_style( 'bloom' );
...
<?php
	if(isset($_POST['add_data_request'])) {
		...
		$post = array(
			...
			'post_status' => 'private', 
			'post_title' => $_POST['challenge-title'], 
			'post_type' => 'data-challenges' // custom type
		);
...
//var_dump($_POST);
			...
			$suggested_data_sets = get_post_meta($post->ID, 'suggested-data-sets', true);

			update_post_meta($postID, 'wpcf-organisation', $_POST['organisation'], $organisation);
			update_post_meta($postID, 'wpcf-suggested-data-sets', $_POST['suggested-data-sets'], $suggested_data_sets);
		}
	}
//
...
//add_filter( 'preview_page_link', 'change_preview_link' );
